...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "ads") {
    (async () => {
      ...
      // On rattache le lot a la recherche dont l'URL partage la meme requete.
      const searchId = matchSearch(msg.url, await get("plan", []), runs);
      await deliver({
        searchId: searchId || null,
        runId: searchId ? runs[searchId] : null,
        url: msg.url,
        page: msg.page,
        total: msg.total,
        source: msg.source,
 ...
 */
function matchSearch(url, plan, runs) {
  try {
    const q = new URL(url).searchParams.get("text") || "";
    for (const j of plan) {
      if (!j.url) continue;
      if ((new URL(j.url).searchParams.get("text") || "") === q) return j.searchId;
    }
